import abc
import numpy as np
from keras.layers import Dense, Dropout, LSTM, Input, Bidirectional, concatenate
...
class GAT_LSTM1(QRRecurrentModel):

    def constructModel(self, **kwargs):
        input1 = Input(shape=(self.trainX1.shape[1], self.trainX1.shape[2]))
        l1 = LSTM(32, return_sequences=True, **kwargs)(input1)
        l1 = Dropout(self.dropout)(l1)
        l1 = LSTM(32, return_sequences=True, **kwargs)(l1)
        l1 = Dropout(self.dropout)(l1)
        l1 = Bidirectional(LSTM(32, activation='relu'), name='bilstm')(l1)
        ...
        ga1 = GAT(20,
                                           ...
                                           attn_kernel_regularizer=l2(5e-4/2))(input2)
        ...
        l3 = Reshape((16, 11))(ga2)
        l3 = LSTM(32, return_sequences=True, **kwargs)(l3)
        l3 = Dropout(self.dropout)(l3)
        l3 = Bidirectional(LSTM(32, activation='relu'), name='bilstm2')(l3)

        added = concatenate([l1, l3], axis=1)
        # added = Flatten()(added)
        total_output = Dense(256, activation='relu')(added)
        total_output = Dense(128, activation='relu')(total_output)
